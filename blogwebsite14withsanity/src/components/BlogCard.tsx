...
export default function BlogCard({ post }: { post: Post }) {
  return (
    <section className="flex flex-col justify-between h-[480px] rounded bg-white dark:bg-dark/40 shadow-md shadow-gray-300 dark:shadow-black/80 group hover:scale-105 transition-transform ease-out duration-700">
      {/* Image Section */}
      ...
      <div className="flex flex-col justify-between gapx-y-4 p-4">
        <h2 className="text-lg line-clamp-2 text-black font-bold leading-tight mb-2">
          {post.title}
        </h2>
        <p className="font-bold text-white line-clamp-3">
          {post.summary}
        </p>
        ...
        <Link
          href={`/blog/${post.slug}`}
          className="block px-4 py-1 text-center bg-pink-700 rounded text-dark font-semibold mt-4"
        >
          Read More
        </Link>
      </div>
